...
searchBar.addEventListener("keyup", (e) => {
    ...
    const getUstensilsStringFromRecipe = (recipe) => {
      let ustensilsString = "";
      recipe.ustensils.forEach((ust) => {
        ustensilsString += `${normalizeValues(ust)}`;
      });
      return ustensilsString;
    };

    const getIngredientsStringFromRecipe = (recipe) => {
      let ingredientsString = "";
      recipe.ingredients.forEach((ing) => {
        ingredientsString += `${normalizeValues(ing.ingredient)}`;
      });
    ...
    for (let i = 0; i < recipes.length; i++) {
      ...
      if(normalizeValues(recipes[i].name).includes(searchString)){
        filtredRecipes.push(recipes[i]);  
        
      }else if(normalizeValues(recipes[i].appliance).includes(searchString)){
        filtredRecipes.push(recipes[i]);
    
      }else if(normalizeValues(recipes[i].description).includes(searchString)){
        filtredRecipes.push(recipes[i]);

      }else if(getIngredientsStringFromRecipe(recipes[i]).includes(searchString)){  
        filtredRecipes.push(recipes[i]);
        
      }else if(getUstensilsStringFromRecipe(recipes[i]).includes(searchString)){
        filtredRecipes.push(recipes[i]);
